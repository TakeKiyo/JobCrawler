...
df = pd.read_csv("unexperienced.csv")
idx = 0
...
for row in df.itertuples():
    if idx==0:
        ...
        OutputList.append([row[2],row[3],row[4],row[5],list(set(nouns) & set(domainList)),row[7],list(set(nouns) & set(TechnologyList)),t])
    else:   
        driver.get(row[3])
        if len(driver.find_elements_by_xpath('//*[@id="select-occupation-type"]/div[1]/span/i')) > 0:
            driver.find_element_by_xpath('//*[@id="select-occupation-type"]/div[1]/span/i').click()
...
df.to_csv("output/unexperienced.csv",encoding='utf_8_sig')
